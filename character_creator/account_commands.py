...
class CmdCharCreate(COMMAND_DEFAULT_CLASS):
    ...
    def func(self):
        ...
        if not self.args:
            self.msg("Usage: erschaffung <charname> [= description]")
            return
        key = self.lhs
        desc = self.rhs
        ...
        if _MAX_NR_CHARACTERS is not None:
            if (
                ...
                self.msg(f"Du kannst nur {_MAX_NR_CHARACTERS} character{plural} erschaffen.")
                return
        ...
        if ObjectDB.objects.filter(db_typeclass_path=typeclass, db_key__iexact=key):
            ...
            # classes.
            self.msg(f"|rLeider gibt es schon den Spieler '|w{key}|r' schon.|n")
            return
        ...
        if desc:
            ...
            new_character.db.desc = "Das ist ein standart Spieler."
        self.msg(
